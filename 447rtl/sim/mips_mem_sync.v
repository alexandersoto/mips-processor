 ...
 *
 * Hacked in a disgusting fashion by Joshua Wise
 *
...
module mips_mem(addr1,data_in1,data_out1,we1,excpt1,allow_kernel1,kernel1,
   ...
   parameter
	    // Data segment
	    data_start   = 32'h00000000,
	    data_words   = 'h400000, /* 16 M */
	    ...
	    // Text segment
	    text_start   = 32'hFF000000,
	    text_words   = 'h4000, /* 64 K */
	    ...
	    stack_top    = 32'hFF100000,
	    stack_words  = 'h4000, /* 64 K */
	    ...
	    // Kernel data segment
	    kdata_start  = 32'hFF200000,
	    kdata_words  = 'h4000, /* 64 K */
   ...
   always@(rst_b)
     if(rst_b == 1'b0)
	  ...
	  $readmemh("mem.fixed.dat", data_seg);
       end
   ...
   always @(rst_b or addr1 or data_in1 or write_mask1 or 
	...
	if((write_mask1 != 32'b0) && (rst_b == 1'b1))
	  ...
	  begin
	     ...
	     if((addr1 >= data_start_w) && (addr1 < data_top_w))
	       begin
		  excpt1 <= 1'b0;
		  kernel1 <= 1'b0;
	       end
   ...
   always @(rst_b or addr2 or data_in2 or write_mask2 or 
	...
	if((write_mask2 != 32'b0) && (rst_b == 1'b1))
	  ...
	  begin
	     ...
	     // Figure out which segment
	     if((addr2 >= data_start_w) && (addr2 < data_top_w))
	       begin
		  excpt2 <= 1'b0;
		  kernel2 <= 1'b0;
	       end
